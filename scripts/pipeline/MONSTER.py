...
def main(argv):
    ...
    # Parse arguments
    try:
        opts, args = getopt.getopt(argv,"hbcv",["input_file=","out_folder=","hits_nb=","neighbours_nb=", "hit_size_treshold=","codon"])
    ...
    for opt, arg in opts:
        if opt == "-h":
            print("\n")
            print("Help section:\n")
            print("This script will apply a pipeline to reconstruct mutational spectrums.\n")
            print("For every given query, it will succesively call different scripts to make") 
            print("the BLAST queries, extract the consensus and the observed (and if possible")
            print("expected) deviations to finally reconstruct the mutation spectrums. To see") 
            print("a detailed description of the pipeline please check the gitlab:") 
            print("https://gitlab.epfl.ch/baffou/mutational-spectrum .  Note that each step of")
            print("the pipeline can be run as a standalone script. Thus you can tweak intermediate")
            print("inputs and outputs as you want or even add some extra steps in between. For") 
            print("the I/O format of this script but also for the sub-scripts please check the")
            ...
            print("-out_folder: (String) Name of the folder where both intermediate and final")
            ...
            print("-b: (flag) Indicates if the step 1 should run in browser mode rather than") 
            print("    with biopython (default=False).")
            print("-v: (flag) Indicates if the browser should be visible (default=False).")  
            print("-c: (flag) Indicate if context should be taken into account (default=False).")
            print("-h: (flag) Opens this Help section.\n")
            print("Outputs:\n")
            print("The script will output 4 different files. Three of them are intermediate results:")
            print("-sequences.csv: (DataFrame) Contains the results of all BLAST queries.")
            print("-logs.csv: (DataFrame) Contains extra informations on queries.")
            print("-deviations.json: (dict) Contains all observed and expected deviations.")
            print("The last one contains the mutational spectrums for each valid initial query: mut_spec.JSON")
            sys.exit()
        elif opt == "-b":
            browser_mode = True
        elif opt == "-c":
    ...
    try:
        ...
        print(f"Impossible to create output folder {out_folder}. Please be sure of your permissions.")
        sys.exit(2)
    # Step 1
    ...
    print("-"*72)
    # Step 3
    step_3_arguments = ["--deviations",out_folder+"/deviations.json","--output",out_folder+"/mut_spec.json"]
    if context:
        step_3_arguments += ["-c"]
    step_3.main(step_3_arguments)
    print("-"*72)
    print(f"Pipeline executed with success! \nOutput files are located in {out_folder}")
